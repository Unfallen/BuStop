(function(){
    ...
    function passengerController(PassengerService, SweetAlert, $ionicModal, $compile,$scope, $state, $stateParams, $http, $cordovaGeolocation) {
        var vm = this;
        ...
        function bookTrip() {
            ...
            PassengerService.bookTrip(data).then(function (response) {
                ...
                $scope.modal.hide();
                localStorage.hasTrip = true;
                vm.hasTrip = true;

                $state.go('passenger.booking', {userId: $stateParams.userId, tripId: response.data.bus_trip_id});
                //$state.transitionTo('passenger.booking', {}, {reload: true, notify:true});
                //$state.reload('passenger.booking');
            });
        }
